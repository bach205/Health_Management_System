...
const DoctorProfile = () => {
    ...
    useEffect(() => {
        if (profile?.id) {
            formProfile.setFieldsValue({
                ...
                speciality: profile.doctor?.speciality || "",
            });
        }
    }, [profile]);
    ...
    const fetchProfile = async () => {
        try {
            if (user?.id) {
                const res = await getDoctorById(Number(user?.id))
                // console.log("res", res)
            ...
            <Flex justify="center" align="center">
                ...
                >
                    ...
                    <Form.Item label="Chức vụ" >
                        <Input disabled value={profile?.role === "doctor" ? "Bác sĩ" : profile?.role === "nurse" ? "Y tá" : "Không xác định"} />
                    </Form.Item>
                    <Form.Item label="Chuyên khoa" >
                        <Input disabled value={profile?.doctor?.speciality || "Không xác định"} />
                    </Form.Item>
